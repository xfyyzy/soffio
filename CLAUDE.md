...
> **Model:** Claude Opus 4.5 — optimized for deep reasoning, extended thinking, and complex code analysis.
...
> conflict. When unsure, leverage extended thinking to analyze trade-offs, then **fail closed** if uncertainty remains.
...
1. **Prioritize correctness over speed.** Encode invariants in types; prefer compile-time checks over runtime
   heuristics. Use extended thinking to verify correctness before proposing changes.
...
3. **Be explicit and auditable.** Every sensitive change MUST be testable, observable, and explainable via diffs, logs,
   and metrics. Document your reasoning chain.
...
5. **Constrain change to the boundary.** Keep domain/core stable; place side effects in adapters/edges unless explicitly
   requested otherwise.
6. **Fail closed.** If correctness is uncertain after deep analysis, stop and request clarification. If silence
   persists, implement the **conservative** option and document assumptions explicitly.
...
## 1) Leveraging Claude Opus 4.5 Capabilities
...
- Safety-critical code review (unsafe blocks, FFI boundaries)
- Multi-file refactoring impact assessment
- Debugging intricate issues with multiple potential root causes
- Designing state machines and type-level invariants

### 1.2 Deep Code Analysis

Before modifying code:

- Trace data flow and ownership through the affected modules
- Identify all callers and dependents of changed interfaces
- Verify invariants are preserved across the change boundary
- Consider edge cases and error propagation paths

### 1.3 Multi-Step Planning
...
- Track progress explicitly through the task list

---

## 2) Preferred Tooling (preinstalled; use these first)
...
Use a single atomic commit when possible. Use the template in §11.

### 5.5 Changelog & Release discipline

- Always identify and label Breaking changes in `CHANGELOG.md`.
- Default behavior: record every codebase-related change under **Unreleased** (either in the same commit as the code change or a dedicated changelog commit).
- Release flow (execute **only when the user explicitly requests a release**):
  1) Update `Cargo.toml` to the user-specified version.
  2) Run the full gate (fmt, clippy, tests, etc.) with `SQLX_TEST_DATABASE_URL=postgres://soffio:soffio_local_dev@127.0.0.1:5432/postgres DATABASE_URL=postgres://soffio:soffio_local_dev@localhost:5432/soffio_dev`; accept snapshot diffs from the version bump.
  3) Update `CHANGELOG.md`: create the new version section, move Unreleased contents into it, and add any missing release details.
  4) After user reconfirms, create the release with `gh`: tag `vX.Y.Z`, title `vX.Y.Z - <brief title>`, notes derived from that version’s changelog entry.
...
- Use extended thinking to verify safety invariants before proposing unsafe code.
...
## 9) Frontend Interaction & Styling
...
## 10) Code Review Checklist (Claude-specific)

Before proposing any change, verify:

- [ ] All affected code paths have been traced and understood
- [ ] Invariants are preserved or explicitly documented as changed
- [ ] Error handling is complete — no silent failures
- [ ] Public API changes are backward-compatible or migration is documented
- [ ] Tests cover the happy path, error paths, and edge cases
- [ ] No unnecessary changes beyond the requested scope
- [ ] Commit message follows the template format
...
## 12) Clarifications, Escalation & Stop Rules

- Ask when necessary for correctness. Provide: the proposed minimal diff, the invariant at risk, and the decision
  needed.
- Use extended thinking to explore alternatives before escalating.
- If any immediate stop condition in §0 triggers, stop and escalate.
- When presenting options, include trade-off analysis with pros/cons for each approach.
