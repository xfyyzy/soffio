...
- Feed service internals are now further split into focused `application/feed` modules (`types`, `service`) behind a stable `application::feed` surface, reducing module coupling without changing feed behavior.
- Render job internals are now further split into focused `application/render/jobs` modules (`payloads`, `process`) behind a stable `render::jobs` surface, reducing payload/process coupling without changing job semantics.
...
- Cache storage internals are now decomposed into focused `cache/store` modules (`l0`, `l1`, `tests`) with `store/mod.rs` reduced to shared types, metrics, and wiring, preserving L0/L1 cache semantics and lock-poison recovery behavior.
- Cache planning internals now use a dedicated `cache/planner` module split (`mod.rs`, `tests.rs`), keeping planning behavior unchanged while reducing monolithic file size.
- Cache consumer internals are now decomposed into focused `cache/consumer` modules (`invalidation`, `warm`, `tests`) with `mod.rs` reduced to orchestration, preserving invalidation and warm-up ordering semantics.
- Cache middleware internals now use a dedicated `cache/middleware` module split (`mod.rs`, `tests.rs`), preserving runtime behavior while reducing maintenance overhead.
...
- CLI command argument internals are now decomposed into focused `crates/soffio-cli/src/args/commands` modules (`api_keys`, `posts`, `pages`, `tags`, `navigation`, `uploads`, `settings`, `jobs`, `snapshots`, `audit`) with `commands.rs` reduced to module wiring and re-exports, preserving command parsing and handler-facing type contracts.
...
- Admin tag application service internals are now decomposed into focused `application/admin/tags` modules (`service`, `queries`, `commands`, `types`) with a stable public `admin::tags` surface, replacing monolithic `tags.rs` without changing tag workflow behavior.
