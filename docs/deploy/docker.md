...
容器入口点为 `soffio`，可通过传入子命令执行一次性任务：

- 渲染全量内容：`docker run --rm <image> renderall --posts --pages`
- 导出静态站点：`docker run --rm -v $(pwd)/export:/out <image> export --file /out/site.toml`
- 导入快照：`docker run --rm -v $(pwd)/backup:/in <image> import --file /in/site.toml`
